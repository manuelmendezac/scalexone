...
const Marketplace: React.FC = () => {
  ...
  const renderCursoCard = (curso: Curso) => (
    <Link to={`/marketplace/producto/${curso.id}`} className="block group h-full">
      <motion.div
      ...
      >
        ...
        <div className="p-6 flex flex-col flex-grow">
          ...
          <div className="mt-auto pt-4 border-t border-white/10 flex justify-between items-center">
            ...
            <button className="bg-amber-600/80 text-white text-center font-bold py-2 px-4 rounded-lg group-hover:bg-amber-500 transition-colors flex items-center justify-center gap-2 text-sm">
              Ver Curso
            </button>
          </div>
        </div>
  ...
  const renderServicioCard = (servicio: Servicio) => {
    ...
    return (
      <Link to={`/marketplace/producto/${servicio.id}`} className="block group h-full">
        <motion.div
          ...
          <div className="p-6 flex flex-col flex-grow">
            ...
            <div className="mt-auto pt-4 border-t border-white/10 flex justify-between items-center">
              ...
              <button className={`${colorScheme.button} text-white font-bold px-6 py-2 rounded-lg text-sm shadow-lg transform transition-all duration-200 hover:scale-105`}>
                {isSuscripcion ? 'Suscribirse' : 'Contratar'}
              </button>
            </div>
          </div>
        </motion.div>
